...
export default function EdpmsDashboardView({
  ...
  const processedStats = useMemo(() => {
    ...
    const sortedPerformers = [...filteredStaffList].sort((a, b) => b.aiScore - a.aiScore)
        ...
        if (format === "csv" || format === "xlsx") {
          ...
          if (activeSource === "checklist") {
            ...
            autoTable(doc, {
              ...
              styles: { fontSize: 8 },
              didParseCell: (data) => {
                if (data.section === 'body' && data.column.index === 2) { // Score column
                  const val = parseFloat(data.cell.raw);
                  if (val >= 850) {
                    data.cell.styles.textColor = [16, 185, 129]; // Emerald Green
                    data.cell.styles.fontStyle = 'bold';
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-16">
      ...
      <div className="relative space-y-6">
        {tabLoading && (
          <div className="absolute inset-0 bg-white z-50 rounded-2xl">
            <div className="sticky top-0 h-[70vh] w-full flex flex-col items-center justify-center">
              ...
              <p className="text-emerald-800 text-sm font-black animate-bounce tracking-wide text-center max-w-sm px-4">
                {funnyMsg}
              </p>
            </div>
          </div>
